...
# we're going to change these
  ...
  poster_session_durations_no_th_ev$numbers_of_posters <- 0
...
#-------------------------------------------------------------------  
 ...
 container_for_this_day_part_with_session_from_while <- 
   container_for_this_day_part %>% 
   mutate(day_part_from_while  = as.character(poster_session_durations_no_th_ev$day_part[i]))
...
# what sessions are not in there?
session_output <- 
x2 %>% 
  group_by(`Session Id`) %>% 
  pull(`Session Id`) %>% 
...
setdiff(session_input, session_output)
...
# what are in the output of this loop that is not in the input?
# duplicates?
all_of_the_day_parts %>% 
  distinct()
